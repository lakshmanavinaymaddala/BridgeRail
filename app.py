...
# Load and Force Numeric Types
df = pd.read_csv('Train_details_22122017.csv')
df.columns = df.columns.str.strip()

# FIX: Convert SEQ and Train No to numbers, turning errors into 'NaN'
df['SEQ'] = pd.to_numeric(df['SEQ'], errors='coerce')
df['Train No'] = pd.to_numeric(df['Train No'], errors='coerce')

# Remove any rows where SEQ or Train No couldn't be converted
